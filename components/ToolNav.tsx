...
export default function ToolNav() {
  ...
  const currentLabel = currentTool ? toolLabel(currentTool, locale) : "";
  ...
  return (
    <>
      ...
      {/* 모바일: 상단 네비게이션 바 (fixed) */}
      <div className="md:hidden fixed top-[52px] left-0 right-0 z-40 bg-white/95 backdrop-blur-sm border-b border-slate-100 px-5 py-2">
        <div className="flex items-center justify-between">
          <Link href="/tools" className="flex items-center gap-1 text-xs font-semibold text-slate-400 active:text-slate-600 py-1">
            ← 목록
          </Link>
          {currentLabel && (
            <span className="text-xs font-bold text-slate-700 truncate max-w-[140px]">{currentTool?.emoji} {currentLabel}</span>
          )}
          <button
            onClick={() => setOpen(!open)}
            className="flex items-center gap-1 text-xs font-semibold text-blue-500 active:text-blue-700 py-1"
          >
            다른 도구 {open ? "▲" : "▼"}
          </button>
        </div>
        ...
        {/* 도구 전환 패널 */}
        {open && (
          <div ref={panelRef} className="mt-2 max-h-[50vh] overflow-y-auto pb-2 relative z-50">
            {TOOL_SECTIONS.map(section => (
              <div key={section.section} className="mb-2">
                <p className="text-[11px] font-bold text-slate-400 px-2 py-1">{section.section}</p>
                <div className="grid grid-cols-2 gap-1">
                  {section.tools.map(tool => (
                    ...
                    >
                      <span className="text-sm">{tool.emoji}</span>
                      <span className="truncate leading-tight">{toolLabel(tool, locale)}</span>
                    </Link>
                  ))}
                </div>
              </div>
