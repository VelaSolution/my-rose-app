...
export default function SimulatorError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => { console.error("Simulator error:", error); }, [error]);

  return (
    <main className="min-h-screen bg-slate-50 flex items-center justify-center px-4">
      <div className="text-center max-w-md">
        <div className="text-5xl mb-4">🧮</div>
        <h1 className="text-xl font-extrabold text-slate-900 mb-2">시뮬레이터 오류</h1>
        <p className="text-slate-500 text-sm mb-6">시뮬레이션을 실행하는 중 문제가 생겼습니다.</p>
